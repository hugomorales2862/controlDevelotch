<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-white tracking-tight glow-cyan">{{ __('Nuevo Rol') }}</h2>
    </x-slot>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                            @foreach($permissions as $perm)
                                <label class="flex items-center space-x-2 bg-[#0B1120] border border-[#1e293b] rounded-lg px-3 py-2 hover:border-[#00f6ff]/30 transition-colors cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $perm->name }}" class="rounded bg-[#0B1120] border-[#1e293b] text-[#00f6ff] focus:ring-[#00f6ff]/20" {{ in_array($perm->name, old('permissions', [])) ? 'checked' : '' }}>
