<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto py-8 px-4">
        <div class="bg-[#0f172a] rounded-2xl shadow-xl border border-[#1e293b] overflow-hidden">
            <div class="p-8">
                <form method="POST" action="{{ route('servers.store') }}" class="space-y-8">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <input id="hostname" name="hostname" type="text" value="{{ old('hostname') }}" required autofocus
                                   ...
                                   placeholder="server01.develotech.app">
                        ...
                        <div>
                            ...
                            <input id="ip_address" name="ip_address" type="text" value="{{ old('ip_address') }}" required
                                   class="block w-full bg-[#0B1120] border border-[#1e293b] focus:border-[#00f6ff] focus:ring focus:ring-[#00f6ff]/20 rounded-xl text-[#00f6ff] font-mono py-4 px-5 transition-all">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <input id="provider" name="provider" type="text" value="{{ old('provider') }}" required
                                ...
                                <input id="region" name="region" type="text" value="{{ old('region') }}"
                                       class="block w-full bg-[#0B1120] border border-[#1e293b] rounded-xl text-white py-4 px-5 text-xs" placeholder="Región">
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-2 grid grid-cols-3 gap-6 bg-[#0B1120] p-6 rounded-2xl border border-white/5">
                            <div>
                                ...
                                <input id="cpu_cores" name="cpu_cores" type="number" value="{{ old('cpu_cores', 1) }}" required
                                       class="block w-full bg-[#0f172a] border border-[#1e293b] rounded-xl text-white py-3 px-4 transition-all">
                            </div>
                            <div>
                                <label for="ram_gb" class="block font-black text-[9px] text-[#00f6ff] uppercase tracking-tighter mb-2 italic">RAM (GB)</label>
                                <input id="ram_gb" name="ram_gb" type="number" value="{{ old('ram_gb', 1) }}" required
                                       class="block w-full bg-[#0f172a] border border-[#1e293b] rounded-xl text-white py-3 px-4 transition-all">
                            </div>
                            <div>
                                ...
                                <input id="storage_gb" name="storage_gb" type="number" value="{{ old('storage_gb', 10) }}" required
                                       class="block w-full bg-[#0f172a] border border-[#1e293b] rounded-xl text-white py-3 px-4 transition-all">
                            </div>
                        </div>
                        ...
                        <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input id="monthly_cost" name="monthly_cost" type="number" step="0.01" value="{{ old('monthly_cost', 0) }}" required
                            ...
                            <div>
                                ...
                                <input id="setup_date" name="setup_date" type="date" value="{{ old('setup_date', date('Y-m-d')) }}" required
                                       class="block w-full bg-[#0B1120] border border-[#1e293b] rounded-xl text-white py-4 px-5 text-xs">
                            </div>
                            <div>
                                <label for="expiry_date" class="block font-semibold text-xs text-slate-400 uppercase tracking-widest mb-2">Fecha de Venc.</label>
                        ...
                        <div class="md:col-span-2">
                            <label for="notes" class="block font-semibold text-xs text-slate-400 uppercase tracking-widest mb-2">Observaciones Técnicas</label>
                            <textarea id="notes" name="notes" rows="3" 
                                      class="block w-full bg-[#0B1120] border border-[#1e293b] focus:border-[#00f6ff] rounded-xl text-slate-300 py-4 px-5 transition-all">{{ old('notes') }}</textarea>
                        </div>
                    </div>

                    <div class="flex items-center justify-end mt-12 pt-8 border-t border-[#1e293b]">
                        <a href="{{ route('servers.index') }}" class="text-xs text-slate-500 hover:text-white mr-8 font-black uppercase tracking-widest transition-colors italic">Cancelar</a>
                        <button type="submit" 
                                class="inline-flex items-center px-10 py-4 bg-gradient-to-r from-[#00f6ff] to-[#3b82f6] hover:from-[#3b82f6] hover:to-[#00f6ff] border border-transparent rounded-2xl font-black text-xs text-[#0B1120] uppercase tracking-tighter transition-all duration-500 shadow-[0_0_20px_rgba(0,246,255,0.2)]">
                            Registrar Servidor
                        </button>
                    </div>
                </form>
