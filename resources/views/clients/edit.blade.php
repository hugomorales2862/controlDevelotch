<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-white tracking-tight">
            {{ __('Editar Cliente:') }} <span class="text-[#00f6ff] glow-cyan">{{ $client->name }}</span>
        </h2>
    </x-slot>
            ...
            <div class="p-8">
                <form method="POST" action="{{ route('clients.update', $client) }}" class="space-y-6">
                    ...
                    <div class="flex items-center justify-end mt-8 border-t border-[#1e293b] pt-6">
                        ...
                        <button type="submit" class="inline-flex items-center px-6 py-3 bg-[#0B1120] border border-[#00f6ff] rounded-xl font-bold text-xs text-[#00f6ff] uppercase tracking-widest hover:bg-[#00f6ff] hover:text-[#0B1120] focus:outline-none transition-all duration-300 shadow-[0_0_15px_rgba(0,246,255,0.2)]">
                            Actualizar Ficha
                        </button>
                    </div>
                </form>
