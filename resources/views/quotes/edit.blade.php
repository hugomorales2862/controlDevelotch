<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-2xl text-white tracking-tight glow-cyan">
            {{ __('Edición de Cotización:') }} <span class="text-[#00f6ff]">{{ $quote->reference }}</span>
        </h2>
    </x-slot>

    <div class="max-w-6xl mx-auto py-8 px-4" x-data="quoteForm('{{ addslashes($quote->quoteable_type) }}|{{ $quote->quoteable_id }}')">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="bg-[#0f172a] rounded-2xl border border-[#1e293b] p-8 shadow-xl relative overflow-hidden">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 md:col-span-2">
                                ...
                                <div>
                                    <label for="valid_until" class="block font-semibold text-xs text-slate-400 uppercase tracking-widest mb-2 italic">Fecha de Validez</label>
                                    <input id="valid_until" name="valid_until" type="date" value="{{ old('valid_until', $quote->valid_until ? $quote->valid_until->format('Y-m-d') : '') }}" required
                                           class="block w-full bg-[#0B1120] border border-[#1e293b] focus:border-[#00f6ff] rounded-xl text-rose-400 font-bold py-4 px-5 transition-all">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" rows="3" 
                                      class="block w-full bg-[#0B1120] border border-[#1e293b] focus:border-[#00f6ff] focus:ring focus:ring-[#00f6ff]/20 rounded-xl text-slate-300 py-4 px-5 transition-all">{{ old('description', $quote->description) }}</textarea>
                        </div>
                    </div>
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="bg-gradient-to-br from-[#0f172a] to-[#1e293b] rounded-2xl border border-[#1e293b] p-8 shadow-2xl sticky top-8">
                        ...
                        <div class="mb-8">
                            <label for="notes" class="block font-semibold text-xs text-slate-500 uppercase tracking-widest mb-4 italic">Observaciones internas / Notas</label>
                            <textarea id="notes" name="notes" rows="3" 
                                      ...
                                      placeholder="Ej: Trato cerrado por teléfono.">{{ old('notes', $quote->notes) }}</textarea>
                        </div>
    ...
    <script>
        function quoteForm(initialValue = '') {
            return {
                ...
                items: {!! json_encode($quote->items->map(function($i) { 
                    return [
