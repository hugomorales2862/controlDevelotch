                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <select name="service_id" class="block mt-1 w-full bg-[#0B1120] border border-[#1e293b] focus:border-[#00f6ff] focus:ring focus:ring-[#00f6ff]/20 rounded-xl text-[#00f6ff] font-bold shadow-sm py-3 px-4" required>
                                @foreach($services as $s)
                                    <option value="{{ $s->id }}" class="bg-[#0B1120]" {{ old('service_id', $subscription->service_id) == $s->id ? 'selected' : '' }}>{{ $s->name }} ({{ $s->duration_days }} días)</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="status" class="block mt-1 w-full bg-[#0B1120] border border-[#1e293b] focus:border-[#00f6ff] focus:ring focus:ring-[#00f6ff]/20 rounded-xl text-white shadow-sm py-3 px-4" required>
                                <option value="active" class="bg-[#0B1120]" {{ old('status', $subscription->status) == 'active' ? 'selected' : '' }}>Activa</option>
                                <option value="suspended" class="bg-[#0B1120]" {{ old('status', $subscription->status) == 'suspended' ? 'selected' : '' }}>Suspendida</option>
                                <option value="canceled" class="bg-[#0B1120]" {{ old('status', $subscription->status) == 'canceled' ? 'selected' : '' }}>Cancelada Permanente</option>
                            </select>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <input class="block mt-1 w-full bg-[#0B1120] border border-[#1e293b] focus:border-[#00f6ff] focus:ring focus:ring-[#00f6ff]/20 rounded-xl text-white shadow-sm py-3 px-4" type="date" name="start_date" value="{{ old('start_date', $subscription->start_date ? $subscription->start_date->format('Y-m-d') : '') }}" required />
                            </div>
                            <div>
                                <label class="block font-medium text-xs text-slate-400 uppercase tracking-widest mb-1">Vencimiento</label>
                                <input class="block mt-1 w-full bg-[#0B1120] border border-[#1e293b] focus:border-[#00f6ff] focus:ring focus:ring-[#00f6ff]/20 rounded-xl text-rose-400 font-bold shadow-sm py-3 px-4" type="date" name="end_date" value="{{ old('end_date', $subscription->end_date ? $subscription->end_date->format('Y-m-d') : '') }}" required />
                            </div>
                        </div>
                    </div>

                    <div class="flex items-center justify-end mt-8 border-t border-[#1e293b] pt-6">
                        <a href="{{ route('subscriptions.index') }}" class="text-sm text-slate-500 hover:text-white mr-6 font-medium transition-colors">Volver</a>
